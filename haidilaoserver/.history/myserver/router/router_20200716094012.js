...
router.get('/userInfo', async(ctx,body) => {
    ...
    if(one_per.status == 1){
      console.log('6666')
      var sql_add = "insert into user(userId,password,telnumber,nickname,avatar,mailbox) values('"+ one_per.userId +"','"+ one_per.password +"','"+ one_per.telnumber +"','"+ one_per.nickname +"','"+ one_per.mailbox +"')"
    ...
    // if(one_per.status == 2){
    //   var one_per = ctx.request.query
    //   var sql_del = "delete from user where username='"+ one_per.username +"'"
    ...
    // if(one_per.status == 3){
    //   var one_per = ctx.request.query
    //   var sql_reset = "update user set name='"+ one_per.name +"',phone='"+ one_per.phone +"',nickname='"+ one_per.nickname +"',sex='"+ one_per.sex +"',address='"+ one_per.address +"',sch_id='"+ one_per.sch_id +"',major_id='"+ one_per.major_id +"',classname='"+ one_per.classname +"',grade='"+ one_per.grade +"',education='"+ one_per.education +"',home_address='"+ one_per.home_address +"',plan='"+ one_per.plan +"'where username='"+ one_per.username +"'"
    //   var results_reset = await query(sql_reset)
    //   console.log('修改成功')
    // }
  })
// 商品信息管理路由
router.get('/goodsInfoMana', async(ctx,body) => {
  var one_per = ctx.request.query
  console.log(one_per)
  // 增加客户信息
  if(one_per.status == 1){
