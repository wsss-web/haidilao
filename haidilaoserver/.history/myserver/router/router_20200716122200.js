...
router.post('/goodsInfoMana', async(ctx,body) => {
  ...
  if(one_per.status == 3){
    var one_per = ctx.request.query
    var sql_reset = "update user set productName='"+ one_per.productName +"',price='"+ one_per.price +"',category='"+ one_per.category +"',stocks='"+ one_per.stocks +"',productPicture='"+ one_per.productPicture +"',description='"+ one_per.description +"' where productNumber='"+ one_per.productNumber +"'"
    var results_reset = await query(sql_reset)
