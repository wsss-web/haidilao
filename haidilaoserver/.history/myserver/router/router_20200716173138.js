...
router.post('/orderFormMana', async(ctx,body) => {
  ...
  if(one_per.flag == 3){
    var one_per = ctx.request.query
    var sql_reset = "update orderForm set userId='"+ one_per.userId +"',productNumber='"+one_per.productNumber+"',generationTime='"+ one_per.generationTime +"',totalPrice='"+ one_per.totalPrice +"',status='"+ one_per.status +"' where userId='"+ one_per.userId +"'&& orderNumber='"+ one_per.orderNumber +"'"
    var results_reset = await query(sql_reset)
